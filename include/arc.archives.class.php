    ...
    function GetPreNext($gtype='')
    {
        ...
        if(count($this->PreNext)<2)
        {
            ...
            if(is_array($preRow))
            {
                ...
                $this->PreNext['pre'] = "<a href='$mlink' class=\"CaseDetailedleft\"></a> ";
                $this->PreNext['preimg'] = "<a href='$mlink'>{$preRow['title']}</a> "; 
            }
            else
            {
                $this->PreNext['pre'] = "<a href='javascript:;' class=\"CaseDetailedleft\"></a> ";
                $this->PreNext['preimg'] ="没有了";
            }
            if(is_array($nextRow))
            {
                $mlink = GetFileUrl($nextRow['id'],$nextRow['typeid'],$nextRow['senddate'],$nextRow['title'],$nextRow['ismake'],$nextRow['arcrank'],
                $nextRow['namerule'],$nextRow['typedir'],$nextRow['money'],$nextRow['filename'],$nextRow['moresite'],$nextRow['siteurl'],$nextRow['sitepath']);
                $this->PreNext['next'] = "<a href='$mlink' class=\"CaseDetailedright\"></a> ";
                $this->PreNext['nextimg'] = "<a href='$mlink'>{$nextRow['title']}</a>";
            }
            else
            {
                $this->PreNext['next'] = "<a href='javascript:;' class=\"CaseDetailedright\"></a> ";
                $this->PreNext['nextimg'] ="没有了";
            }
        }
        if($gtype=='pre')
        {
